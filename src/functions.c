...
char game_setting_options[7][22] = {
    "- Level: 1",
    "- Lines: 0",
    "- See Next: YES",
    "- Ghost Hint: NO",
...
static void initSRAM()
{
    ...
    if (test_value != SRAM_CHECKSUM_VALUE)
    {
        ...
        // Initialize game config settings
...
void triggerSelectedGameSetting(int button_pressed, int direction)
{
    ...
    case 4:
        ...
        sprintf(game_setting_options[4], "- Shift Grid: %s", game_config.shift_grid ? "YES" : "NO ");
        drawGameSettingsMenu();
        break;
    case 5:
        if (direction)
        {
            ...
            if (direction < 0 && game_config.tetromino_set == 0)
                game_config.tetromino_set = 7;
            else
                game_config.tetromino_set = (game_config.tetromino_set + direction) % 8;
        }
        ...
        sprintf(game_setting_options[5], "- Shape Set: %s", shape_name);
        drawGameSettingsMenu();
        break;
    case 6:
        if (button_pressed)
